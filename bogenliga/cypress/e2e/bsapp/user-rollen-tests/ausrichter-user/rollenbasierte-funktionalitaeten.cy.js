describe('testvontest', function () {
  ...
  it('DSBMitglied bearbeiten', function () {
    cy.visit('http://localhost:4200/#/verwaltung/dsbmitglieder/1200');
    //cy.get('[data-cy="TABLE.ACTIONS.EDIT"]').should('be.visible').click({ multiple: true});
    cy.get('[data-cy=detail-vorname-feld]').click();
    cy.get('[data-cy=detail-vorname-feld]').type('VornameTestTest');
    cy.get('#dsbMitgliedUpdateButton').click();
    cy.get('#dsbMitgliedUpdateButton > .action-btn-circle').click();
  });
