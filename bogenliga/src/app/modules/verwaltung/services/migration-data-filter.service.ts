...
import {fromPayload, fromPayloadArray, fromPayloadArrayWithLimit} from '../mapper/trigger-mapper';
...
export class MigrationFilterService extends DataProviderService {
  ...
  public findLimitedData(data_limit: number = 5): Promise<BogenligaResponse<TriggerDO[]>> {
    // return promise
    // sign in success -> resolve promise
    // sign in failure -> reject promise with result
    return new Promise((resolve, reject) => {
      this.restClient.GET<Array<VersionedDataTransferObject>>(this.getUrl())
          .then((data: VersionedDataTransferObject[]) => {
            resolve({result: RequestResult.SUCCESS, payload: fromPayloadArrayWithLimit(data, data_limit)});
          }, (error: HttpErrorResponse) => {
