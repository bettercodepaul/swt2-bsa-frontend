...
export class WettkampfOfflineSyncService extends DataProviderService {
  ...
  public async createWettkampfDummyData(): Promise<void> {
    const offlineWettkampfArray: OfflineWettkampf[] = [];
    for (let i = 0; i < 4; i++) {
      const offlineWettkampf: OfflineWettkampf = {
        id: 30 + i,
        datum: '2017-12-30',
        ausrichter: '2',
        beginn: '12:5' + i,
  ...
  public async goOnlineSync(wettkampfID: number ): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      */
      ...
      await this.restClient.POST(new UriBuilder().fromPath(this.getUrl()).path(`wettkampf/${wettkampfID}/sync`).build(), payload);
      } catch (error) {
        console.error(error);
        reject(error);
      }
