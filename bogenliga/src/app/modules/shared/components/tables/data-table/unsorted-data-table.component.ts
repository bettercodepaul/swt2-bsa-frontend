...
@Component({
  selector:    'bla-unsorted-data-table',
  templateUrl: './data-table.component.html',
  styleUrls:   ['./data-table.component.scss'],
  providers:   [TranslatePipe, TruncationPipe]
})
export class UnsortedDataTableComponent extends CommonComponent implements OnInit, OnChanges {
  ...
  public getSortingIcon(column: TableColumnConfig): string {
    ...
    let icon = '';
    if (sortingClasses.indexOf('sortable') > 0) {

      if (sortingClasses.indexOf('unsorted') > -1) {
        icon = 'sort';
      } else if (sortingClasses.indexOf('unsorted') > -1) {
        icon = 'sort-up';
      } else if (sortingClasses.indexOf('unsorted') > -1) {
        icon = 'sort-down';
      }
