...
import {VereinDTO} from '../../types/datatransfer/vereine-dto.class';
...
export class VereineComponent extends CommonComponent implements OnInit {
  ...
  private handleLoadTableRowsSuccess(response: Response<VereinDO[]>): void {
    this.rows = []; // reset array to ensure change detection
    this.rows = toTableRows(response.payload);
    this.loading = false;
  }

  private handleLoadTableRowsFailure(response: Response<VereinDO[]>): void {
    this.rows = [];
    this.loading = false;
  }
